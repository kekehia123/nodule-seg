...
class FCDenseNet_3d(nn.Module):
    ...
    def __init__(self, num_input_features = 1,                
                 num_classes = 1000,
                 growth_rate  = 16,
                 drop_rate = 0.2,
        ...
        super(FCDenseNet_3d, self).__init__()
        ...
        self.add_module('conv0', nn.Conv3d(num_input_features, num_init_features, kernel_size=(3,3,3), padding=(1,1,1), bias=False))
        ...
        self.finalconv = nn.Conv3d(current_channels, 1, kernel_size=1, stride=1, bias=True)
...
def FCDensenet_3d(num_classes, dense_bn_size=None, dense_compression=1.0):
    model = FCDenseNet_3d(num_classes=num_classes, num_input_features = 2, 
                       growth_rate=12, num_layers_per_blocks=4, num_pool=3,
...
#         self.add_module('norm', nn.BatchNorm2d(num_input_features))
        ...
        if bn_size and isinstance(bn_size, int):
            self.add_module('conv_bottleneck', nn.Conv3d(num_input_features, bn_size * growth_rate, 
                                          kernel_size=1, stride=1, bias=False))
#             self.add_module('norm_bottleneck', nn.BatchNorm2d(bn_size * growth_rate))
            self.add_module('norm_bottleneck', nn.InstanceNorm3d(bn_size * growth_rate, affine=True))
            self.add_module('relu_bottleneck', nn.ReLU(inplace=True))
            in_channels = bn_size * growth_rate

        self.add_module('conv', nn.Conv3d(in_channels, growth_rate,
                               kernel_size=(1,3,3), stride=1, padding=(0,1,1), bias=False))
        
        if drop_rate > 0:
            self.add_module('drop', nn.Dropout3d(drop_rate, inplace=True))
...
#         self.add_module('norm', nn.BatchNorm2d(num_input_features))
        ...
        self.add_module('conv', nn.Conv3d(num_input_features, num_output_features,
                               kernel_size=(2,1,1), stride=1, bias=False))
        
        if drop_rate > 0:
            self.add_module('drop', nn.Dropout3d(drop_rate))
...
class _Transition_Up(nn.Module):
    ...
    def __init__(self, upsample_channels: int, skip_channels = None):
        ...
        self.add_module('upconv', nn.ConvTranspose3d(self.upsample_channels, self.upsample_channels,
                                      kernel_size=(2,3,3), stride=2, padding=0, bias=True))
        self.add_module('concat', CenterCropConcat_3d())
...
class CenterCropConcat_3d(nn.Module):
    def forward(self, x, y):
        if x.shape[0] != y.shape[0]:
            raise ValueError(f'x and y inputs contain a different number of samples')
        z = min(x.size(2), y.size(2))
        height = min(x.size(3), y.size(3))
        width = min(x.size(4), y.size(4))
        
        #print(x.size(), y.size())

        x = self.center_crop_3d(x, z, height, width)
        y = self.center_crop_3d(y, z, height, width)

        res = torch.cat([x, y], dim=1)
        return res

    @staticmethod
    def center_crop_3d(x, target_z, target_height, target_width):
        current_z = x.size(2)
        current_height = x.size(3)
        current_width = x.size(4)
        min_z = (current_z - target_z) // 2
        min_h = (current_width - target_width) // 2
        min_w = (current_height - target_height) // 2
        return x[:, :, min_z:(min_z + target_z), min_w:(min_w + target_height), min_h:(min_h + target_width)]