...
joint_transformer['train'] = transforms.Compose([    
    ...
    joint_transforms_3d.JointAnnoCenterCrop_3d(128),
    joint_transforms_3d.JointRandomHorizontalFlip_3d()
    ])
...
dataset['train'] = io_utils_lidc3d.lidc3d(DATA_PATH, 'train', [num_z, crop_size, crop_size], split_files = 'train.npy',
                                    joint_transform=joint_transformer['train'])
dataloader['train'] = torch.utils.data.DataLoader(dataset['train'], batch_size=batch_size, 
                                                  shuffle=True, num_workers=2)
# ==== Val ====
dataset['val'] = io_utils_lidc3d.lidc3d(DATA_PATH, 'val', [num_z, crop_size, crop_size], split_files = 'val.npy',
                                  joint_transform=joint_transformer['val'])
dataloader['val'] = torch.utils.data.DataLoader(dataset['val'], batch_size=1, 
                                                shuffle=False, num_workers=2)
# ==== Test ====
dataset['test'] = io_utils_lidc3d.lidc3d(DATA_PATH, 'test', [num_z, crop_size, crop_size], split_files = 'test.npy',
...
print("Train: %d" %len(dataloader['train'].dataset.label_names))
print("Val: %d"   %len(dataloader['val'].dataset.label_names))
print("Test: %d"  %len(dataloader['test'].dataset.label_names))
...
model = Tiramisu_3d.FCDenseNet_3d(num_input_features=1, num_classes=2, dense_bn_size=4, dense_compression=0.5).cuda()
...
experiment_name = 'DenseU_3d_CenterAnnoCrop_horFlip'
